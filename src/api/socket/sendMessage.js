...
module.exports = function (defaultFuncs, api, ctx) {
    ...
    return async function sendMessage(msg, threadID, callback, replyToMessage, isSingleUser) {
        ...
        // only attachment sends were routed through the E2EE bridge, so every plain-text
        // bot reply inside an E2EE DM silently vanished (sent over MQTT, never delivered).
        // Now: if the DM is E2EE-connected, ALL sends (text or attachment) go through the
        // Noise WebSocket / Signal Protocol bridge. Only non-E2EE DM attachments fall back
        // to OldMessage; everything else falls through to the normal MQTT path below.
        var useE2EE = isSingleUser && api.e2ee && typeof api.e2ee.isConnected === "function" && api.e2ee.isConnected();
        var useE2EEGroup = !isSingleUser && api.e2ee && typeof api.e2ee.isKnownE2EEGroup === "function" &&
            api.e2ee.isKnownE2EEGroup(String(threadID)) && typeof api.e2ee.isConnected === "function" && api.e2ee.isConnected();
        useE2EE = useE2EE || useE2EEGroup;
